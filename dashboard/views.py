...
from .export_utils import export_orders_to_csv, export_customers_to_csv, export_contacts_to_csv, export_inventory_to_csv
...
# API Endpoints
@user_passes_test(superuser_required)
def api_inventory_detail(request, item_id: int):
    """API endpoint to get inventory item details for order forms"""
    try:
        item = get_object_or_404(Inventory, pk=item_id)
        return JsonResponse({
            'success': True,
            'product_name': f"{item.brand} {item.model_name}",
            'product_model': item.model_name,
            'price': float(item.price),
            'stock_quantity': item.stock_quantity,
            'availability': item.get_availability_status()
        })
...
def export_contacts(request):
    ...
    return export_contacts_to_csv(contacts)
...
def export_inventory(request):
    ...
    return export_inventory_to_csv(inventory_items)
...
def reports_dashboard(request):
    ...
    orders_last_7_days = Order.objects.filter(order_date__date__gte=last_7_days).count()
    
    # Revenue statistics
    total_revenue = Order.objects.aggregate(total=Sum('total_amount'))['total'] or 0
    revenue_last_30_days = Order.objects.filter(
        order_date__date__gte=last_30_days
    ...
    order_status_stats = Order.objects.values('order_status').annotate(
        count=Count('id')
    ).order_by('-count')
    
    # Payment status breakdown
    payment_status_stats = Order.objects.values('payment_status').annotate(
        count=Count('id')
    ).order_by('-count')
    ...
    low_stock_items = Inventory.objects.filter(stock_quantity__lte=5).count()
    out_of_stock_items = Inventory.objects.filter(stock_quantity=0).count()
...
def order_update(request, order_id: int):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Only allow processing if payment is received
            if updated_order.order_status == 'processing' and updated_order.payment_status != 'paid':
                messages.error(request, 'Cannot process order until payment is received.')
                return render(request, 'order_form.html', {'form': form, 'order': order})
            updated_order.save()  # This triggers email notification via model's save()
            messages.success(request, 'Order updated successfully.')
            return redirect(reverse('orders_list'))
    else:
        form = OrderForm(instance=order)
